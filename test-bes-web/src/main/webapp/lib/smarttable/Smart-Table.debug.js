...
(function (angular) {
    ...
    angular.module('smartTable.table', ['smartTable.column', 'smartTable.utilities', 'smartTable.directives', 'smartTable.filters', 'ui.bootstrap.pagination.smartTable'])
        ...
        .controller('TableCtrl', ['$scope', 'Column', '$filter', '$parse', 'ArrayUtility', 'DefaultTableConfiguration', function (scope, Column, filter, parse, arrayUtility, defaultConfig) {
            ...
            this.changeItemsByPage = function (itemsByPage){
            	scope.itemsByPage = itemsByPage;
            	this.changePage({page : scope.currentPage});
            };
        }]);
})(angular);
...
angular.module("partials/pagination.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("partials/pagination.html",
        "<div class=\"pagination\">\n" +
        	"    <span ng-show=\"itemsByPages!=undefined&&itemsByPage!=undefined\">每页显示<select ng-model=\"itemsByPage\" ng-options=\"x for x in itemsByPages\" ng-change=\"changeItemsByPage()\"></select>条</span>\n" +
            "    <ul>\n" +
            ...
            "    <span ng-show=\"numPages>10\" class=\"skipDiv\">到<input type=\"text\" class=\"form-control\" ng-model=\"shiftPage\"/>页<button class=\"but1\" ng-click=\"goPage(shiftPage)\">确定</button></span>\n" +
            "</div> ");
}]);
...
angular.module("partials/smartTable.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("partials/smartTable.html",
        "<table class=\"smart-table\">\n" +
            ...
            "            <div pagination-smart-table=\"\" num-pages=\"numberOfPages\" max-size=\"maxSize\" current-page=\"currentPage\" items-by-page=\"itemsByPage\" items-by-pages=\"itemsByPages\"></div>\n" +
            "        </td>\n" +
            "    </tr>\n" +
            "    </tfoot>\n" +
            "</table>\n" +
        ...
        .directive('paginationSmartTable', ['paginationConfig', 'templateUrlList', function (paginationConfig, templateUrlList) {
            return {
                ...
                link: function (scope, element, attrs, ctrl) {
                    ...
                    scope.changeItemsByPage = function() {
                    	if(ctrl!=undefined){
                            ctrl.changeItemsByPage(scope.itemsByPage);
                        }
                    }
                }
            };
