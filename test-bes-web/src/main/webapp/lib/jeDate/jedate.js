...
//            obj.style.left = rect.left + (pos ? 0 :jeDt.scroll(1)) + "px";//OLD
            var myLeft = (rect.right + obj.offsetWidth / 1.5 >= doc.documentElement["clientWidth"] ? doc.documentElement["clientWidth"] - obj.offsetWidth : rect.left + (pos ? 0 : jeDt.scroll(1)) )+ "px";
            ...
            /*为了医嘱开立的特殊日期格式修改的代码*/
            if(document.getElementById(QD(this.config.dateCell)[0].id+'x')!=null&&
            		document.getElementById(QD(this.config.dateCell)[0].id+'x')!=undefined&&
            		document.getElementById(QD(this.config.dateCell)[0].id+'x')!=null)
            {
	            var position = document.getElementById(QD(this.config.dateCell)[0].id+'x').getBoundingClientRect();
//	            obj.style.left = jeDt.scroll()+position.left + "px";//OLD
	            obj.style.left = ( position.left + rect.right + obj.offsetWidth / 1.5 >= doc.documentElement["clientWidth"] ? doc.documentElement["clientWidth"] - obj.offsetWidth : position.left + rect.left + (pos ? 0 : jeDt.scroll(1)) )+ "px";
	            rect=position;
	            tops =  (rect.bottom + obj.offsetHeight / 1.5 <= jeDt.winarea()) ?
	                    rect.bottom - 1 : rect.top > obj.offsetHeight / 1.5 ? rect.top - obj.offsetHeight + 1 :jeDt.winarea() - obj.offsetHeight;
            ...
            var objm = document.getElementById('jedatebox');
            var fn = function(ev){
            	jeDt.stopmp(ev);
            	jeDt.html(objm, "");
                jeDt.shdeCell(true);
            }
            var addEvent = function(objx,xEvent,fn){
	            if(objx.attachEvent){
	                objx.attachEvent('on'+xEvent,fn);
	              }else{
	                objx.addEventListener(xEvent,fn,false);
	              }
            };
            addEvent(objm,"mousewheel",fn);
            addEvent(objm,"DOMMouseScroll",fn);
